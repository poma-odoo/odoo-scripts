...
if __name__ == '__main__':
    ...
    for js_file in fileinput.input():
        js_file=js_file.strip()
        url = re.sub(r'.*(?P<module>/\w+)(?P<path>/static/*)', r'\g<module>\g<path>', js_file)
        # print(url)
        # path_split = js_file.split("/")
        # root = ''
        # part = path_split.pop(0)
        # while part != 'static':
        #     root = part
        #     part = path_split.pop(0)
        # url2 = '/%s/%s/%s' % (root,part, '/'.join(path_split))
        # print(url2)
        with open(js_file, "r+") as f:
            content = f.read()
            print(transpile_javascript(url,content))
        input('next')
